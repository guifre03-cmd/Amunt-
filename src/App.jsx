...
['4d9f','4 de 9 amb folre',4,9,93,24600,155,5,1510,1820,'f']
 ...
 // ['3d8s','3 de 8 aixecat per sota',3,8,95,31400,100,5,2340,2600,'s'],
  ...
  ['5d9f','5 de 9 amb folre',5,9,97,43000,184,6,2595,3125,'f'],['4d9af','4 de 9 amb folre i agulla',4,9,98,46500,180,7,2955,3285,'fa'],['3d9af','3 de 9 amb folre i agulla',3,9,98,50200,170,7,3100,3445,'fa'],['4d9','4 de 9 sense folre',4,9,98,54200,140,7,3405,4105,''],['2d8','2 de 8 sense folre',2,8,98,58500,105,7,3575,4310,''],
...
function Hub({club,g,avail,now,proposal,openProposal,train,calendar,advance,start,reset}){return <main className="wrap"><p className="eyebrow">TEMPORADA {g.season} · SETMANA {g.week}/16</p><h1>{club.name}</h1><p>{club.town}</p><div className="stats">{[['Membres',g.members],['Prestigi',g.prestige],['Energia',g.energy+'%'],['Castells',avail.length]].map(x=><div className="card stat" key={x[0]}><strong>{x[1]}</strong><span>{x[0]}</span></div>)}</div>{proposal&&<button className="proposal" onClick={openProposal}>🧑‍🏫 <span><small>PROPOSTA DE L’EQUIP TÈCNIC</small><strong>Comencem a treballar el {proposal.name}?</strong></span></button>}{now&&<button className="event" onClick={start}>{now.icon} {now.name}<small>ANAR A PLAÇA</small></button>}<div className="menu"><button onClick={train}>🏋️<strong>Assaig</strong><small>{g.tests} proves</small></button><button onClick={calendar}>📅<strong>Calendari</strong></button><button onClick={advance}>🌙<strong>Passar setmana</strong></button></div><section className="card"><h2>Llibreta del cap de colla</h2>{avail.map(c=><div className="row" key={c.id}><strong>{c.name}</strong><span>{feel(g.prep[c.id]||0)}</span></div>)}</section><button className="secondary" onClick={reset}>Esborrar partida</button></main>}
function Proposal({c,accept,later}){return <main className="hero"><div className="panel"><div className="big">🧑‍🏫</div><h2>Comencem a treballar el {c.name}?</h2><p>Requisits assolits: {c.prestige} de prestigi i {c.people} membres.</p><button onClick={accept}>COMENCEM A ASSAJAR-LO</button><button className="secondary" onClick={later}>ARA NO</button></div></main>}
function Choose({g,avail,go,back}){const sorted=[...avail].sort((a,b)=>b.unloaded-a.unloaded);return <main className="wrap"><button className="secondary" onClick={back}>← Tornar</button><h1>Assaig</h1><div className="notice">Et queden <strong>{g.tests} de 6 proves</strong>.</div><div className="castle-grid">{sorted.map(c=><button className="castle-card" disabled={g.tests<=0} onClick={()=>go(c)} key={c.id}><strong>{c.name}</strong><span className={'status '+(g.prep[c.id]>=68?'good':g.prep[c.id]>=42?'medium':g.prep[c.id]>=22?'fair':'off')}>{feel(g.prep[c.id]||0)}</span></button>)}</div></main>}
function Training({c,color,prep,done}){const st=useMemo(()=>stages(c),[c]),[step,setStep]=useState(1),[stress,setStress]=useState(10),[descending,setDescending]=useState(false),[best,setBest]=useState(1),ended=useRef(false),objective=Math.max(2,Math.min(st.length-2,Math.floor(2+(prep/100)*(st.length-3))));useEffect(()=>{const timer=setInterval(()=>setStress(s=>clamp(s+Math.random()*2.5+c.difficulty*.012+step*.2-prep*.012)),800);return()=>clearInterval(timer)},[step,c,prep]);useEffect(()=>{if(stress>=99&&!ended.current){ended.current=true;done({gain:1,cost:4,best:st[best-1]?.label,result:'Caiguda'})}},[stress]);useEffect(()=>{if(!descending)return;const timer=setTimeout(()=>{if(step<=1&&!ended.current){ended.current=true;const ratio=best/st.length,bonus=best>objective?2:0;done({gain:Math.max(2,Math.round(ratio*10)+bonus),cost:ratio>.9?3:ratio>.55?2:1,best:st[best-1]?.label,result:'Desmuntada correctament'})}else{setStress(s=>clamp(s+Math.random()*3+c.difficulty*.01));setStep(s=>Math.max(1,s-1))}},750);return()=>clearTimeout(timer)},[descending,step]);const up=()=>{const beyond=Math.max(0,step-objective);setStress(s=>clamp(s+3+c.difficulty*.035+step*.45+beyond*6-prep*.025));setStep(s=>{const n=Math.min(st.length,s+1);setBest(b=>Math.max(b,n));return n})};const hold=()=>{const ok=Math.random()*100<prep+38-step*4;setStress(s=>clamp(s+(ok?-(4+Math.random()*4):(2+Math.random()*4))))};return <main className="wrap"><h2>Prova de {c.name}</h2><div className="notice">Objectiu recomanat: <strong>{st[objective]?.label}</strong>. Pots forçar més amunt.</div><div className="local"><header>LOCAL D’ASSAIG</header><Tower c={c} rows={st.slice(0,step)} color={color} stress={stress}/><div className="bubble">{descending?'Desmuntant: '+(st[step-1]?.label||'Pinya'):stress>86?'No ho veig clar!':stress>68?'Tremola molt!':stress>48?'Està treballant...':st[step]?.label||'Prova completa'}</div></div><div className="controls">{!descending?<><button disabled={step>=st.length} onClick={up}>⬆ AMUNT</button><button onClick={hold}>⏸ AGUANTEM</button><button onClick={()=>setDescending(true)}>⬇ DESMUNTEM</button></>:<><button disabled>BAIXANT</button><button onClick={hold}>⏸ AGUANTEM</button><button disabled>FASE {step}</button></>}</div></main>}
function Summary({d,next}){return <main className="hero"><div className="panel"><h2>{d.name}</h2><p>Millor fase: <strong>{d.best}</strong></p><p>Resultat: <strong>{d.result}</strong></p><p>Preparació: <strong>{d.before} → {d.after}</strong></p><p>Millora: <strong>+{d.gain}</strong></p><button onClick={next}>CONTINUAR L’ASSAIG</button></div></main>}
function ActSummary({delta,next}){return <main className="hero"><div className="panel"><h2>Final de l’actuació</h2><div className="big">{delta>=0?'👏':'🧣'}</div><p>{delta>0?`${delta} persones noves s’han apuntat.`:delta<0?`${Math.abs(delta)} persones han marxat de la colla.`:'La colla manté els membres.'}</p><button onClick={next}>TORNAR A LA COLLA</button></div></main>}
function Calendar({g,back,start}){return <main className="wrap"><button className="secondary" onClick={back}>← Tornar</button><h1>Calendari</h1>{EVENTS.map(e=><button className="card calendar" disabled={e.week!==g.week||g.attended.includes(e.name)} onClick={()=>start(e)} key={e.name}>Setmana {e.week} · {e.icon} {e.name}</button>)}</main>}
function Act({event,run,avail,g,attempt,finish}){const pool=(run.phase==='pillar'?avail.filter(c=>c.pillar):avail.filter(c=>!c.pillar&&!run.used.includes(c.id)&&(g.prep[c.id]||0)>=22)).sort((a,b)=>b.unloaded-a.unloaded);return <main className="wrap"><h1>{event.icon} {event.name}</h1><p className="round">{run.phase==='castells'?`Ronda ${run.round}/5 · ${run.valid}/3 castells`:run.phase==='pillar'?'Ronda de pilars':'Actuació acabada'}</p><section className="card">{run.results.map((r,i)=><div className="row" key={i}><span>{r.name}</span><strong>{r.res}</strong></div>)}{run.pillar&&<div className="row"><span>{run.pillar.name}</span><strong>{run.pillar.res}</strong></div>}</section>{run.phase!=='done'?<div className="castle-grid">{pool.map(c=><button className="castle-card" onClick={()=>attempt(c,run.phase==='pillar')} key={c.id}><strong>{c.name}</strong><span>{feel(g.prep[c.id]||0)}</span></button>)}</div>:<button onClick={finish}>TANCAR ACTUACIÓ</button>}</main>}
function Season({g,next}){return <main className="hero"><div className="panel"><h1>🏆 Temporada {g.season-1}</h1><p>Els castells alts han perdut preparació. Els pots recuperar als assajos.</p><button onClick={next}>NOVA TEMPORADA</button></div></main>}
